...
- Matière plate : filet 1 px, liseré haut 1 px sur chaque plaque, rayon 0 partout. Aucun dégradé, aucune ombre, aucun flou. La seule matière animée est la pile de crêtes, en fond du hero.
- Motion calm : les crêtes s'illuminent l'une après l'autre, le câblage de la carte se trace une fois au chargement, le flux descend le long des pistes, et tout le reste répond au survol. Les nombres ne bougent jamais.
...
Sous le premier écran, deux sections seulement : les cinq accès en liste à filets, puis la matrice 27 × 14 en grille `gap: 1px`.
...
**cretes** (le fond du hero) : un `<canvas>` 2D en `position: absolute`, `aria-hidden`, `pointer-events: none`, derrière le titre, le champ et la mesure — et pas derrière la carte, qui est opaque et le cacherait.

**La référence est le *ridgeline plot*, dit *joyplot***, dont le nom vient de la pochette d'Unknown Pleasures (Peter Saville, 1979) : un relevé des impulsions du pulsar PSR B1919+21, chaque courbe tracée sous la précédente et la masquant. C'est la forme canonique pour comparer beaucoup de distributions à la fois, et c'est la question du site : cent douze hooks, et ce que chacun prend.

Anatomie : une crête par hook, dix-huit prises régulièrement dans la pile des cent douze, du moins cher en bas au plus cher en haut. En abscisse le rang du prélèvement, en ordonnée la densité — combien de mesures de ce hook tombent là. Soixante-quatre pas, lissés deux fois sur un noyau de cinq : assez pour qu'un taux fixe devienne un pic dessinable, pas assez pour l'aplatir en ondulation. Chaque courbe est **remplie de `{colors.canvas}`**, ce qui la fait masquer celles du dessous : c'est ce remplissage, et lui seul, qui produit la silhouette. Trait 1 px `{colors.border-strong}` à 90 % ; la crête active passe à `{colors.ink-muted}` à 100 %, une toutes les 420 ms, de bas en haut. Une ligne en `{typography.mono-sm}` la nomme en bas à droite (« une crête par hook, 112 hooks mesurés, ce que chacun prend »). Les textes sont protégés en creusant les **rectangles de ligne** obtenus du `Range` de chaque nœud de texte, jamais la boîte du bloc. Sous `prefers-reduced-motion`, la pile est peinte une fois, complète et immobile ; l'onglet caché l'arrête ; le canvas est redimensionné par `ResizeObserver`.

**Ce que ce fond n'est pas.** Deux fonds ont été refusés avant celui-ci, et les deux refus ont appris quelque chose. Des pistes orthogonales qui poussaient toutes seules : « trop AI generated dans le style » — la forme était celle de n'importe quel site technique. Un semis de points du corpus : juste, mais sans style, un voile sans figure. La règle qui en sort : le fond de ce site est fait de ce que ce site mesure, **et il doit avoir une silhouette qu'on reconnaît de loin**.
