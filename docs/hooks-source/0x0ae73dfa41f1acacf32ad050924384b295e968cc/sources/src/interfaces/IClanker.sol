...
interface IClanker is IOwnerAdmins {
    struct TokenConfig {
        ...
        // cc0strategy: total supply to mint at launch (18 decimals). 0 => default 100B
        // (Clanker.TOKEN_SUPPLY) for backward compatibility. The deployer mints EXACTLY this
        // amount; the pool's starting price is unaffected (it is f(tick) only) — supply only
        // scales liquidity depth. Bounded by Clanker.MIN_SUPPLY / MAX_SUPPLY. Appended LAST so
        // the on-chain + ABI decode of every existing field is unchanged.
        uint256 supply;
    }
    ...
    /// @notice When the requested launch supply is outside [MIN_SUPPLY, MAX_SUPPLY], or the
    ///         post-extension pool supply would be zero
    error InvalidSupply();
