...
abstract contract ClankerHookV2 is BaseHook, IClankerHookV2 {
    ...
    // cc0strategy-v2: the protocol-fee carve is configurable per-deployment instead of a fixed 20%.
    // `MAX_PROTOCOL_FEE_NUMERATOR` preserves the upstream ceiling; the live numerator is
    // `protocolFeeNumerator`, set once in the constructor. cc0.company deploys its hooks with 0 so
    // the full LP fee reaches the locker and is split 75/15/10 there (platform's 10% included),
    // rather than carving a slice at the hook. Kept configurable so a protocol-fee hook stays
    // deployable if ever needed.
    uint256 public constant MAX_PROTOCOL_FEE_NUMERATOR = 200_000; // ceiling: 20% of the LP fee
    uint256 public immutable protocolFeeNumerator;
    int128 public constant FEE_DENOMINATOR = 1_000_000; // Uniswap 100% fee

    uint24 public protocolFee;

    // cc0strategy-v2: raised when a hook is constructed with a protocol fee above the ceiling.
    error ProtocolFeeNumeratorTooHigh();
    ...
    constructor(
        ...
        // cc0strategy-v2: bound the protocol-fee carve to the upstream ceiling (20% of the LP fee).
        if (_protocolFeeNumerator > MAX_PROTOCOL_FEE_NUMERATOR) {
            revert ProtocolFeeNumeratorTooHigh();
        }
        factory = _factory;
    ...
    // function to set the protocol fee to `protocolFeeNumerator` of the lp fee (0 for cc0.company)
    function _setProtocolFee(uint24 lpFee) internal {
        protocolFee = uint24(uint256(lpFee) * protocolFeeNumerator / uint128(FEE_DENOMINATOR));
    }
