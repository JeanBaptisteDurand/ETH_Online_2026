...
interface IInternalSwapPool {
    ...
    /**
     * The price {Oracle} this hook records pre-swap observations into.
     *
     * @dev Already exposed at runtime by {InternalSwapPool}'s `IOracle public immutable oracle`,
     * so declaring it here costs the hooks nothing. It lets the {BidWall} -- which holds no
     * oracle reference of its own and already reaches the hook through this address -- consult
     * the same TWAP the {InternalSwapPool} prices its fills against.
     */
