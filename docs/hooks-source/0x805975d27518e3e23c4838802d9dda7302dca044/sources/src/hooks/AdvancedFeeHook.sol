...
contract AdvancedFeeHook is IHooks {
        ...
        returns (uint256 spent)
    {
        ...
        try poolManager.swap(
            ...
            if (got <= 0 || paid >= 0) return 0;
            poolManager.take(key.currency0, BURN, uint256(uint128(got)));
    ...
    function _payCreatorShare(
        ...
        if (!feeIsToken && c.burnBps > 0) {
            ...
            sharedToPlatform = c.taxBps == 0
                ? 0
                : (unburnable * LaunchFees.platformCut(c.taxBps)) / c.taxBps;
