...
import {IBonkerHookV2PoolExtension} from "./interfaces/IBonkerHookV2PoolExtension.sol";
...
import {IBonkerHookV2} from "./interfaces/IBonkerHookV2.sol";
...
abstract contract BonkerHookV2 is BaseHook, IBonkerHookV2 {
    ...
    IBonkerPoolExtensionAllowlist public immutable poolExtensionAllowlist;
    address public immutable weth;

    mapping(PoolId => bool) public bonkerIsToken0;
    ...
    constructor(
        ...
        factory = _factory;
        poolExtensionAllowlist = IBonkerPoolExtensionAllowlist(_poolExtensionAllowlist);
        weth = _weth;
    }
    ...
    function _initializePoolExtensionData(
        ...
        if (_poolExtension != address(0)) {
            ...
            IBonkerHookV2PoolExtension(_poolExtension).initializePreLockerSetup(
                poolKey, bonkerIsToken0[poolKey.toId()], poolExtensionData
            );
        ...
        onlyFactory
    {
        ...
        if (poolExtension[poolKey.toId()] != address(0)) {
            IBonkerHookV2PoolExtension(poolExtension[poolKey.toId()]).initializePostLockerSetup(
                poolKey, locker[poolKey.toId()], bonkerIsToken0[poolKey.toId()]
            );
    ...
    function _runMevModule(
        ...
        if (mevModuleOperational(poolKey.toId())) {
            ...
            bool disableMevModule = IBonkerMevModule(mevModule[poolKey.toId()]).beforeSwap(
                poolKey, swapParams, bonkerIsToken0[poolKey.toId()], poolSwapData.mevModuleSwapData
            );
    ...
    function _runPoolExtensionHelper(
        ...
        IBonkerHookV2PoolExtension(poolExtension[poolKey.toId()]).afterSwap(
            poolKey, swapParams, delta, bonkerIsToken0[poolKey.toId()], swapData
        );
    }
    ...
    function _afterSwap(
        ...
        if (isExactInput && swappingForBonker || !isExactInput && !swappingForBonker) {
            if (bonkerIsToken0[poolKey.toId()]) {
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBonkerHook).interfaceId
            || interfaceId == type(IBonkerHookV2).interfaceId;
    }
