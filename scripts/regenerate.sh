...
echo "== 2/13 fixtures de parite TypeScript <-> Python =="
bash scripts/regen-fixtures.sh

echo "== 3/13 tableau « what we found » du README =="
PYTHONPATH=engine python3 -m tare.dataset.stats --write-readme

echo "== 4/13 tableau « what the graph reveals » du README =="
( cd engine && python3 -m tare.graph.readme --write-readme )

echo "== 5/13 analyse des sources =="
...
echo "== 9/13 jeu embarque par le site =="
node apps/web/scripts/build-dataset.mjs

echo "== 10/13 declaration : les hooks emettent-ils HookSwap ou HookFee ? =="
...
echo "== 11/13 couverture du registre officiel =="
...
echo "== 12/13 chiffres de la porte de remplacement =="
...
echo "== 13/13 texte de soumission (il lit le releve de l'etape 10) =="
